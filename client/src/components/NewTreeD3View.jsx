...
export function NewTreeD3View(props) {
  ...
  const filterChart = (e) => {
    ...
    data.forEach((d) => {
      if (value != '' && (d.memberInfo.fullName.toLowerCase().includes(value.toLowerCase()) || d.spouseInfo.fullName.toLowerCase().includes(value.toLowerCase()))) {
              ...
              d.width
                    ...
                    <div style="font-family: 'Inter', sans-serif;background-color:${color};  margin-left:-1px;width:${d.width - 2}px;height:${d.height - imageDiffVert}px;border-radius:10px;border: 1px solid #E4E2E9">
                        ...
                        <div style="margin-top:${
                          -imageDiffVert - 20
                        }px;">   <img src="${buildImageForView(
                          d.data.memberInfo.photo,
                          d.data.memberInfo.gender,
                          d.data.id
                        )}" style="margin-left:${12}px;border-radius:100px;width:40px;height:40px;" />
                        <img src="${buildImageForView(
                          d.data.spouseInfo.photo,
                          d.data.spouseInfo.gender,
                          d.data.id
                        )}" style="margin-left:${5}px;border-radius:100px;width:40px;height:40px;" />
                        </div>
...
/><button onClick={()=>orgChartObj.fullscreen()}>Fit to the screen</button>
<button onClick={()=>orgChartObj.exportImg()}>Export Current</button>
<button onClick={()=>orgChartObj.exportImg({full:true})}>Export Full</button>
<button onClick={()=>orgChartObj.exportSvg()}>Export SVG</button>
<button onClick={()=>downloadPdf(orgChartObj)}>Export PDF</button>
<button onClick={()=>orgChartObj.expandAll().fit()}>expandAll</button>
<button onClick={()=>orgChartObj.collapseAll().fit()}>collapseAll</button>
<button onClick={()=>orgChartObj.layout('left').render().fit()}>Left</button>
<button  onClick={()=>orgChartObj.layout('top').render().fit()}>Top</button>
<div id="d3Org"></div></>;
}
